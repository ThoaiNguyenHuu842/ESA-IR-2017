...
import be.vanoosten.esa.CachingTokenStream;
import be.vanoosten.esa.ConceptSimilarity;
...
public class MainIndexer {
	private static Logger log = Logger.getLogger(MainIndexer.class);
    ...
    private static void searchForQuery(final QueryParser parser, final IndexSearcher searcher, final String queryString, final IndexReader indexReader) throws ParseException, IOException {
        ...
        log.info(String.format("%d hits voor \"%s\"", topDocs.totalHits, queryString));
        for (ScoreDoc sd : topDocs.scoreDocs) {
            log.info(String.format("doc %d score %.2f shardIndex %d title \"%s\"", sd.doc, sd.score, sd.shardIndex, indexReader.document(sd.doc).get(TITLE_FIELD)));
        }
    }
